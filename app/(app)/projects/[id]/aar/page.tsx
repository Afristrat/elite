  ...
  return (
    <div className="space-y-6">
      <p className="text-on-surface-variant text-sm">
        After Action Review de <span className="text-on-surface font-medium">{project.title}</span>
      ...
      {!canSubmit ? (
        <div className="bg-surface-container border border-border/10 rounded-xl p-8 text-center">
          <p className="text-muted-foreground text-sm">AAR disponible après la décision du comité</p>
        ...
        <div className="space-y-6">
          {/* Description */}
          <div className="bg-na-primary-container/10 border border-primary/20 rounded-xl p-4">
            <div className="flex items-start gap-3">
              <span className="text-xl mt-0.5">🪖</span>
              <div>
                <p className="text-sm font-semibold text-na-primary-container">Méthode AAR — US Army (1970s)</p>
            ...
            <div className="space-y-3">
              ...
              <div className="space-y-3">
                {allResponses.map((r) => {
                  ...
                  return (
                    <div key={r.id} className="bg-surface-container border border-border/10 rounded-xl p-4 space-y-2">
                      ...
                      {Object.entries(resp).map(([, value], i) => (
                        value ? (
                          <div key={i} className="pl-3 border-l border-border">
                            <p className="text-xs text-on-surface-variant leading-relaxed">{String(value)}</p>
                          </div>
                        ) : null
                      ))}
                    </div>
